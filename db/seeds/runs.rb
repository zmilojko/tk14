p 'Creating runs'

@amf_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @amf,
                           number: '56',
                           times: '{ "final": 804.5 }'
@amf_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @amf,
                           number: '56',
                           times: '{ "final": 873.7 }'
@itp_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @itp,
                           number: '59',
                           times: '{ "final": 816.2 }'
@itp_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @itp,
                           number: '59',
                           times: '{ "final": 873 }'
@ehf_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @ehf,
                           number: '53',
                           times: '{ "final": 826.8 }'
@ehf_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @ehf,
                           number: '53',
                           times: '{ "final": 876.7 }'
@ebs_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @ebs,
                           number: '55',
                           times: '{ "final": 864.4 }'
@ebs_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @ebs,
                           number: '55',
                           times: '{ "final": 887.8 }'
@rvc_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @rvc,
                           number: '57',
                           times: '{ "final": 845.8 }'
@rvc_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @rvc,
                           number: '57',
                           times: '{ "final": 910.3 }'
@arp_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @arp,
                           number: '63',
                           times: '{ "final": 846.3 }'
@arp_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @arp,
                           number: '63',
                           times: '{ "final": 910.2 }'
@mhs_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @mhs,
                           number: '68',
                           times: '{ "final": 826 }'
@mhs_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @mhs,
                           number: '68',
                           times: '{ "final": 936.9 }'
@css_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @css,
                           number: '67',
                           times: '{ "final": 855.2 }'
@css_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @css,
                           number: '67',
                           times: '{ "final": 910 }'
@hhg_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @hhg,
                           number: '65',
                           times: '{ "final": 854.1 }'
@hhg_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @hhg,
                           number: '65',
                           times: '{ "final": 931.8 }'
@kkn_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @kkn,
                           number: '69',
                           times: '{ "final": 861.5 }'
@kkn_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @kkn,
                           number: '69',
                           times: '{ "final": 930.9 }'
@sgs_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @sgs,
                           number: '60',
                           times: '{ "final": 867.5 }'
@sgs_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @sgs,
                           number: '60',
                           times: '{ "final": 927.7 }'
@hlf_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @hlf,
                           number: '66',
                           times: '{ "final": 919 }'
@hlf_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @hlf,
                           number: '66',
                           times: '{ "final": 942.9 }'
@lpp_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @lpp,
                           number: '62',
                           times: '{ "final": 906.5 }'
@lpp_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @lpp,
                           number: '62',
                           times: '{ "final": 960.8 }'
@dnc_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @dnc,
                           number: '54',
                           times: '{ "final": 918.4 }'
@dnc_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @dnc,
                           number: '54',
                           times: '{ "final": 1006.7 }'
@apf_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @apf,
                           number: '64',
                           times: '{ "final": 1006.9 }'
@apf_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @apf,
                           number: '64',
                           times: '{ "final": 1171 }'
@sbf_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @sbf,
                           number: '61',
                           times: '{ "final": 1040.4 }'
@sbf_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @sbf,
                           number: '61',
                           times: '{ "final": 1194.9 }'
@mkl_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @mkl,
                           number: '71',
                           times: '{ "final": 1119.6 }'
@mkl_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @mkl,
                           number: '71',
                           times: '{ "final": 1192.3 }'
@ahf_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @ahf,
                           number: '51',
                           times: '{ "final": 1262.3 }'
@ahf_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @ahf,
                           number: '51',
                           times: ''
@tnf_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @tnf,
                           number: '70',
                           times: ''
@tnf_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @tnf,
                           number: '70',
                           times: ''
@por_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @por,
                           number: '58',
                           times: ''
@por_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @por,
                           number: '58',
                           times: ''
@spc_sp4_day1 = Run.create! race: @sp4_day1,
                           user: @spc,
                           number: '52',
                           times: ''
@spc_sp4_day2 = Run.create! race: @sp4_day2,
                           user: @spc,
                           number: '52',
                           times: ''
@jhn_sp4j_day1 = Run.create! race: @sp4j_day1,
                           user: @jhn,
                           number: '74',
                           times: '{ "final": 808.2 }'
@jhn_sp4j_day2 = Run.create! race: @sp4j_day2,
                           user: @jhn,
                           number: '74',
                           times: '{ "final": 892.8 }'
@acf_sp4j_day1 = Run.create! race: @sp4j_day1,
                           user: @acf,
                           number: '72',
                           times: '{ "final": 909.6 }'
@acf_sp4j_day2 = Run.create! race: @sp4j_day2,
                           user: @acf,
                           number: '72',
                           times: '{ "final": 982.6 }'
@vlf_sp4j_day1 = Run.create! race: @sp4j_day1,
                           user: @vlf,
                           number: '73',
                           times: '{ "final": 936.5 }'
@vlf_sp4j_day2 = Run.create! race: @sp4j_day2,
                           user: @vlf,
                           number: '73',
                           times: '{ "final": 1029.1 }'
@khf_sp6_day1 = Run.create! race: @sp6_day1,
                           user: @khf,
                           number: '78',
                           times: '{ "final": 1076.3 }'
@khf_sp6_day2 = Run.create! race: @sp6_day2,
                           user: @khf,
                           number: '78',
                           times: '{ "final": 1191.6 }'
@cts_sp6_day1 = Run.create! race: @sp6_day1,
                           user: @cts,
                           number: '75',
                           times: '{ "final": 1129.1 }'
@cts_sp6_day2 = Run.create! race: @sp6_day2,
                           user: @cts,
                           number: '75',
                           times: '{ "final": 1174.1 }'
@tkf_sp6_day1 = Run.create! race: @sp6_day1,
                           user: @tkf,
                           number: '76',
                           times: '{ "final": 1127.8 }'
@tkf_sp6_day2 = Run.create! race: @sp6_day2,
                           user: @tkf,
                           number: '76',
                           times: '{ "final": 1207.9 }'
@ton_sp6_day1 = Run.create! race: @sp6_day1,
                           user: @ton,
                           number: '80',
                           times: '{ "final": 1156.7 }'
@ton_sp6_day2 = Run.create! race: @sp6_day2,
                           user: @ton,
                           number: '80',
                           times: '{ "final": 1225.2 }'
@rkf_sp6_day1 = Run.create! race: @sp6_day1,
                           user: @rkf,
                           number: '87',
                           times: '{ "final": 1148.7 }'
@rkf_sp6_day2 = Run.create! race: @sp6_day2,
                           user: @rkf,
                           number: '87',
                           times: '{ "final": 1288.4 }'
@vlf2_sp6_day1 = Run.create! race: @sp6_day1,
                           user: @vlf2,
                           number: '90',
                           times: '{ "final": 1176.3 }'
@vlf2_sp6_day2 = Run.create! race: @sp6_day2,
                           user: @vlf2,
                           number: '90',
                           times: '{ "final": 1266 }'
@jtc_sp6_day1 = Run.create! race: @sp6_day1,
                           user: @jtc,
                           number: '77',
                           times: '{ "final": 1191.9 }'
@jtc_sp6_day2 = Run.create! race: @sp6_day2,
                           user: @jtc,
                           number: '77',
                           times: '{ "final": 1279.8 }'
@jkf_sp6_day1 = Run.create! race: @sp6_day1,
                           user: @jkf,
                           number: '84',
                           times: '{ "final": 1239 }'
@jkf_sp6_day2 = Run.create! race: @sp6_day2,
                           user: @jkf,
                           number: '84',
                           times: '{ "final": 1259.6 }'
@idl_sp6_day1 = Run.create! race: @sp6_day1,
                           user: @idl,
                           number: '85',
                           times: '{ "final": 1204.4 }'
@idl_sp6_day2 = Run.create! race: @sp6_day2,
                           user: @idl,
                           number: '85',
                           times: '{ "final": 1330 }'
@mbg_sp6_day1 = Run.create! race: @sp6_day1,
                           user: @mbg,
                           number: '81',
                           times: '{ "final": 1233.1 }'
@mbg_sp6_day2 = Run.create! race: @sp6_day2,
                           user: @mbg,
                           number: '81',
                           times: '{ "final": 1372.3 }'
@mcs_sp6_day1 = Run.create! race: @sp6_day1,
                           user: @mcs,
                           number: '82',
                           times: '{ "final": 1275.8 }'
@mcs_sp6_day2 = Run.create! race: @sp6_day2,
                           user: @mcs,
                           number: '82',
                           times: '{ "final": 1371.9 }'
@amf_sp6_day1 = Run.create! race: @sp6_day1,
                           user: @amf,
                           number: '83',
                           times: '{ "final": 1271.6 }'
@amf_sp6_day2 = Run.create! race: @sp6_day2,
                           user: @amf,
                           number: '83',
                           times: '{ "final": 1407.4 }'
@mcc_sp6_day1 = Run.create! race: @sp6_day1,
                           user: @mcc,
                           number: '79',
                           times: '{ "final": 1341 }'
@mcc_sp6_day2 = Run.create! race: @sp6_day2,
                           user: @mcc,
                           number: '79',
                           times: '{ "final": 1520.9 }'
@abp_sp6_day1 = Run.create! race: @sp6_day1,
                           user: @abp,
                           number: '86',
                           times: '{ "final": 1430.3 }'
@abp_sp6_day2 = Run.create! race: @sp6_day2,
                           user: @abp,
                           number: '86',
                           times: '{ "final": 1619.4 }'
@atf_sp6_day1 = Run.create! race: @sp6_day1,
                           user: @atf,
                           number: '89',
                           times: '{ "final": 1686.4 }'
@atf_sp6_day2 = Run.create! race: @sp6_day2,
                           user: @atf,
                           number: '89',
                           times: '{ "final": 1758.1 }'
@lpp_sp6_day1 = Run.create! race: @sp6_day1,
                           user: @lpp,
                           number: '88',
                           times: '{ "final": 1315.6 }'
@lpp_sp6_day2 = Run.create! race: @sp6_day2,
                           user: @lpp,
                           number: '88',
                           times: ''
@phf_sp8_day1 = Run.create! race: @sp8_day1,
                           user: @phf,
                           number: '92',
                           times: '{ "final": 1536.8 }'
@phf_sp8_day2 = Run.create! race: @sp8_day2,
                           user: @phf,
                           number: '92',
                           times: '{ "final": 1620 }'
@rsf_sp8_day1 = Run.create! race: @sp8_day1,
                           user: @rsf,
                           number: '102',
                           times: '{ "final": 1537.1 }'
@rsf_sp8_day2 = Run.create! race: @sp8_day2,
                           user: @rsf,
                           number: '102',
                           times: '{ "final": 1642.8 }'
@akg_sp8_day1 = Run.create! race: @sp8_day1,
                           user: @akg,
                           number: '100',
                           times: '{ "final": 1555.8 }'
@akg_sp8_day2 = Run.create! race: @sp8_day2,
                           user: @akg,
                           number: '100',
                           times: '{ "final": 1638.5 }'
@cws_sp8_day1 = Run.create! race: @sp8_day1,
                           user: @cws,
                           number: '93',
                           times: '{ "final": 1609.5 }'
@cws_sp8_day2 = Run.create! race: @sp8_day2,
                           user: @cws,
                           number: '93',
                           times: '{ "final": 1679 }'
@hhg_sp8_day1 = Run.create! race: @sp8_day1,
                           user: @hhg,
                           number: '91',
                           times: '{ "final": 1588.5 }'
@hhg_sp8_day2 = Run.create! race: @sp8_day2,
                           user: @hhg,
                           number: '91',
                           times: '{ "final": 1743.1 }'
@mtg_sp8_day1 = Run.create! race: @sp8_day1,
                           user: @mtg,
                           number: '97',
                           times: '{ "final": 1637.8 }'
@mtg_sp8_day2 = Run.create! race: @sp8_day2,
                           user: @mtg,
                           number: '97',
                           times: '{ "final": 1753 }'
@vhf_sp8_day1 = Run.create! race: @sp8_day1,
                           user: @vhf,
                           number: '99',
                           times: '{ "final": 1695 }'
@vhf_sp8_day2 = Run.create! race: @sp8_day2,
                           user: @vhf,
                           number: '99',
                           times: '{ "final": 1793.3 }'
@egl_sp8_day1 = Run.create! race: @sp8_day1,
                           user: @egl,
                           number: '101',
                           times: '{ "final": 1686.1 }'
@egl_sp8_day2 = Run.create! race: @sp8_day2,
                           user: @egl,
                           number: '101',
                           times: '{ "final": 1902 }'
@svh_sp8_day1 = Run.create! race: @sp8_day1,
                           user: @svh,
                           number: '104',
                           times: '{ "final": 1941.5 }'
@svh_sp8_day2 = Run.create! race: @sp8_day2,
                           user: @svh,
                           number: '104',
                           times: '{ "final": 2062 }'
@mtp_sp8_day1 = Run.create! race: @sp8_day1,
                           user: @mtp,
                           number: '95',
                           times: '{ "final": 2232.3 }'
@mtp_sp8_day2 = Run.create! race: @sp8_day2,
                           user: @mtp,
                           number: '95',
                           times: '{ "final": 1921.4 }'
@klf_sp8_day1 = Run.create! race: @sp8_day1,
                           user: @klf,
                           number: '103',
                           times: '{ "final": 2331.9 }'
@klf_sp8_day2 = Run.create! race: @sp8_day2,
                           user: @klf,
                           number: '103',
                           times: ''
@fhn_sp8_day1 = Run.create! race: @sp8_day1,
                           user: @fhn,
                           number: '98',
                           times: '{ "final": 2598.3 }'
@fhn_sp8_day2 = Run.create! race: @sp8_day2,
                           user: @fhn,
                           number: '98',
                           times: ''
@amr_sp8_day1 = Run.create! race: @sp8_day1,
                           user: @amr,
                           number: '96',
                           times: ''
@amr_sp8_day2 = Run.create! race: @sp8_day2,
                           user: @amr,
                           number: '96',
                           times: ''
@jbn_sp8_day1 = Run.create! race: @sp8_day1,
                           user: @jbn,
                           number: '94',
                           times: ''
@jbn_sp8_day2 = Run.create! race: @sp8_day2,
                           user: @jbn,
                           number: '94',
                           times: ''
@hin_spu_day1 = Run.create! race: @spu_day1,
                           user: @hin,
                           number: '106',
                           times: '{ "final": 3003.5 }'
@hin_spu_day2 = Run.create! race: @spu_day2,
                           user: @hin,
                           number: '106',
                           times: '{ "final": 2912 }'
@kzs_spu_day1 = Run.create! race: @spu_day1,
                           user: @kzs,
                           number: '108',
                           times: '{ "final": 2946.2 }'
@kzs_spu_day2 = Run.create! race: @spu_day2,
                           user: @kzs,
                           number: '108',
                           times: '{ "final": 3101 }'
@mtg_spu_day1 = Run.create! race: @spu_day1,
                           user: @mtg,
                           number: '109',
                           times: '{ "final": 3134.2 }'
@mtg_spu_day2 = Run.create! race: @spu_day2,
                           user: @mtg,
                           number: '109',
                           times: '{ "final": 3220.6 }'
@tsg_spu_day1 = Run.create! race: @spu_day1,
                           user: @tsg,
                           number: '110',
                           times: '{ "final": 3810 }'
@tsg_spu_day2 = Run.create! race: @spu_day2,
                           user: @tsg,
                           number: '110',
                           times: '{ "final": 3931.5 }'
@alf_spu_day1 = Run.create! race: @spu_day1,
                           user: @alf,
                           number: '107',
                           times: ''
@alf_spu_day2 = Run.create! race: @spu_day2,
                           user: @alf,
                           number: '107',
                           times: ''
@ksg_spu_day1 = Run.create! race: @spu_day1,
                           user: @ksg,
                           number: '105',
                           times: ''
@ksg_spu_day2 = Run.create! race: @spu_day2,
                           user: @ksg,
                           number: '105',
                           times: ''
@crs_md12_day1 = Run.create! race: @md12_day1,
                           user: @crs,
                           number: '111',
                           times: '{ "final": 2546 }'
@crs_md12_day2 = Run.create! race: @md12_day2,
                           user: @crs,
                           number: '111',
                           times: '{ "final": 3026.5 }'
@rcf_md12_day1 = Run.create! race: @md12_day1,
                           user: @rcf,
                           number: '112',
                           times: '{ "final": 2847.9 }'
@rcf_md12_day2 = Run.create! race: @md12_day2,
                           user: @rcf,
                           number: '112',
                           times: '{ "final": 3544.7 }'
@ads_md12_day1 = Run.create! race: @md12_day1,
                           user: @ads,
                           number: '115',
                           times: '{ "final": 3697.6 }'
@ads_md12_day2 = Run.create! race: @md12_day2,
                           user: @ads,
                           number: '115',
                           times: '{ "final": 4211.4 }'
@psf_md12_day1 = Run.create! race: @md12_day1,
                           user: @psf,
                           number: '116',
                           times: '{ "final": 5518.2 }'
@psf_md12_day2 = Run.create! race: @md12_day2,
                           user: @psf,
                           number: '116',
                           times: ''
@rrg_md12_day1 = Run.create! race: @md12_day1,
                           user: @rrg,
                           number: '114',
                           times: '{ "final": 6497.1 }'
@rrg_md12_day2 = Run.create! race: @md12_day2,
                           user: @rrg,
                           number: '114',
                           times: ''
@ppc_md12_day1 = Run.create! race: @md12_day1,
                           user: @ppc,
                           number: '113',
                           times: ''
@ppc_md12_day2 = Run.create! race: @md12_day2,
                           user: @ppc,
                           number: '113',
                           times: ''
@pkf_md6_day1 = Run.create! race: @md6_day1,
                           user: @pkf,
                           number: '119',
                           times: '{ "final": 3027.6 }'
@pkf_md6_day2 = Run.create! race: @md6_day2,
                           user: @pkf,
                           number: '119',
                           times: '{ "final": 3576.9 }'
@asr_md6_day1 = Run.create! race: @md6_day1,
                           user: @asr,
                           number: '123',
                           times: '{ "final": 3353.2 }'
@asr_md6_day2 = Run.create! race: @md6_day2,
                           user: @asr,
                           number: '123',
                           times: '{ "final": 4034.5 }'
@mvc_md6_day1 = Run.create! race: @md6_day1,
                           user: @mvc,
                           number: '120',
                           times: '{ "final": 3301 }'
@mvc_md6_day2 = Run.create! race: @md6_day2,
                           user: @mvc,
                           number: '120',
                           times: '{ "final": 4159.2 }'
@gbp_md6_day1 = Run.create! race: @md6_day1,
                           user: @gbp,
                           number: '122',
                           times: '{ "final": 3670.4 }'
@gbp_md6_day2 = Run.create! race: @md6_day2,
                           user: @gbp,
                           number: '122',
                           times: '{ "final": 4468.5 }'
@hma_md6_day1 = Run.create! race: @md6_day1,
                           user: @hma,
                           number: '126',
                           times: '{ "final": 3968.8 }'
@hma_md6_day2 = Run.create! race: @md6_day2,
                           user: @hma,
                           number: '126',
                           times: '{ "final": 4196.9 }'
@jss_md6_day1 = Run.create! race: @md6_day1,
                           user: @jss,
                           number: '118',
                           times: '{ "final": 3648.7 }'
@jss_md6_day2 = Run.create! race: @md6_day2,
                           user: @jss,
                           number: '118',
                           times: '{ "final": 4705.4 }'
@jef_md6_day1 = Run.create! race: @md6_day1,
                           user: @jef,
                           number: '117',
                           times: '{ "final": 3493.6 }'
@jef_md6_day2 = Run.create! race: @md6_day2,
                           user: @jef,
                           number: '117',
                           times: '{ "final": 4904.6 }'
@kkf_md6_day1 = Run.create! race: @md6_day1,
                           user: @kkf,
                           number: '124',
                           times: '{ "final": 4091.8 }'
@kkf_md6_day2 = Run.create! race: @md6_day2,
                           user: @kkf,
                           number: '124',
                           times: '{ "final": 4355.6 }'
@slf_md6_day1 = Run.create! race: @md6_day1,
                           user: @slf,
                           number: '127',
                           times: '{ "final": 4426.8 }'
@slf_md6_day2 = Run.create! race: @md6_day2,
                           user: @slf,
                           number: '127',
                           times: '{ "final": 5035.3 }'
@gzc_md6_day1 = Run.create! race: @md6_day1,
                           user: @gzc,
                           number: '121',
                           times: '{ "final": 4659.7 }'
@gzc_md6_day2 = Run.create! race: @md6_day2,
                           user: @gzc,
                           number: '121',
                           times: '{ "final": 5634.5 }'
@paf_md6_day1 = Run.create! race: @md6_day1,
                           user: @paf,
                           number: '125',
                           times: '{ "final": 5868.2 }'
@paf_md6_day2 = Run.create! race: @md6_day2,
                           user: @paf,
                           number: '125',
                           times: ''

